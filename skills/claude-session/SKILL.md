...
### 陷阱12：多行文本粘贴后不发送（2026-04-27 补充）

**现象**：
- 发送多行文本（27行代码）后，Claude Code TUI 显示 `[Pasted text #1 +27 lines]`
- 状态显示 IDLE，但 Claude 没有收到消息，一直在等待
- `wait_for_idle` 立即返回 "idle"，但实际任务未开始

**根因**：
1. tmux 的 `send-keys -l` 对长文本会触发 bracketed paste 模式
2. Claude Code TUI 显示 "[Pasted text]" 等待确认
3. `send()` 发送 Enter 时，paste 可能还未完成，Enter 被终端"吃掉"
4. 状态机检测到 `❯` prompt，误判为 IDLE
5. `wait_for_idle` 立即返回，但文本实际未发送

**三层防御修复**（已合并到代码）：

1. **tmux_interface.py**: 多行文本发送后延迟 10 秒再按 Enter
   ```python
   if "\n" in text:
       time.sleep(10.0)
...
2. **output_parser.py**: 检测 `[Pasted text]` 模式，返回 `INPUTTING` 状态
   ```python
...
3. **manager.py**: 检测到 INPUTTING 状态时自动补发 Enter
   ```python
   if (
...
**诊断方法**：
1. 直接查看 tmux pane：`tmux capture-pane -t <session> -p | tail -20`
2. 如果看到 `[Pasted text #N +M lines]`，说明粘贴未完成
3. 检查日志中是否有 "Pasted text detected in INPUTTING state"

**临时手动修复**：
如果遇到此问题卡住，手动在 tmux 中按 Enter：
```bash
...
**预防**：
- 多行文本发送后等待足够时间（10秒）
- 检查状态是否为 INPUTTING 而非仅看 tmux prompt
- 如果 `wait_for_idle` 立即返回但任务未开始，检查实际 tmux 内容
