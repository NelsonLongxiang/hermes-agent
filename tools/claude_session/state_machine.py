...
class StateMachine:
    ...
    def transition(self, new_state: str) -> Optional[StateTransition]:
        ...
        with self._lock:
            ...
            # Log unexpected transitions for debugging (but still allow them,
            # since TUI parsing is imperfect and strict enforcement would break)
            if not is_valid_transition(old_state, new_state):
                import logging
                logging.getLogger(__name__).warning(
                    "Unexpected state transition: %s → %s (allowed for robustness)",
                    old_state, new_state,
                )
