    ...
    async def _handle_media_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not self._should_process_message(update.message):
            if self._should_observe_unmentioned_group_message(update.message):
                ...
                _observe_event = self._build_message_event(
                    _m, _observe_type, update_id=update.update_id
                )
                if _m.caption:
                    _observe_event.text = self._clean_bot_trigger_text(_m.caption) or _m.caption
                await self._cache_observed_media(_m, _observe_event)
