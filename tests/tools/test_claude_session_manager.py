...
"""
    ...
    @patch("tools.claude_session.manager.AdaptivePoller")
    @patch("tools.claude_session.manager.TmuxInterface")
    def test_start_polls_once_before_returning_state(self, MockTmux, MockPoller, manager):
        """start() should not return the initial DISCONNECTED placeholder state."""
        mock_tmux = MagicMock()
        mock_tmux.session_exists.return_value = True
...
"""
        ...
        def poll_now_side_effect():
            manager._sm.transition("IDLE")
        mock_poller.poll_now.side_effect = poll_now_side_effect
        MockPoller.return_value = mock_poller

        result = manager.start(workdir="/tmp/test")

        mock_poller.start.assert_called_once()
        mock_poller.poll_now.assert_called_once()
...
class TestSend:
    ...
    @patch("tools.claude_session.manager.time.sleep")
    def test_multiline_send_delays_enter_outside_lock(self, mock_sleep, manager):
        """Multi-line paste must wait 10s before Enter, without tmux_interface sleeping."""
        manager._session_active = True
        manager._tmux = MagicMock()
        manager._sm.transition("IDLE")

        result = manager.send("line 1\nline 2")

        assert result["sent"] is True
        manager._tmux.send_keys.assert_called_once_with("line 1\nline 2", enter=False)
        mock_sleep.assert_called_once_with(10.0)
        manager._tmux.send_special_key.assert_called_once_with("Enter")
...
class TestWaitForIdle:
    ...
    def test_patrol_returns_progress_without_waiting_full_timeout(self, manager):
        """Long waits must yield progress so Hermes agent loop is not blocked for minutes."""
        manager._session_active = True
        manager._sm.transition("THINKING")
        manager._current_turn = Turn(
            turn_id=1, start_time=time.monotonic(), end_time=None,
        ...
        with patch.dict(os.environ, {"HERMES_CLAUDE_SESSION_PATROL_INTERVAL": "1"}):
        ...
        assert elapsed < 3.0
        assert manager._wait_state is not None
